...
void MotorCMDTask(void)
{
    ...
    // motor_data.Controller.input_torque = cmd_rx.torque[(can_tx_id - 1) % 4] * motor_data.components.encoder->dir * MOTOR_REF_CURRENT_LIMIT / 16384.0;
    /*1000hz反馈*/
    cmd_tx.position = motor_data.components.encoder->raw;
    cmd_tx.velocity = motor_data.components.encoder->vel_estimate * motor_data.components.encoder->dir * 16384 / MOTOR_VEL_LIMIT;
    cmd_tx.torque   = motor_data.components.foc->i_q * motor_data.components.encoder->dir * 16384 / MOTOR_REF_CURRENT_LIMIT;
