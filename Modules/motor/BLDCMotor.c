...
void SetPwm(FOC_DATA *foc)
{
    ...
    // if (foc->dtc_a > DUTY_LIMIT) foc->dtc_a = DUTY_LIMIT;
    // if (foc->dtc_b > DUTY_LIMIT) foc->dtc_b = DUTY_LIMIT;
    // if (foc->dtc_c > DUTY_LIMIT) foc->dtc_c = DUTY_LIMIT;
...
void Svpwm_Sector(FOC_DATA *foc)
{
    ...
    foc->dtc_a = CLAMP((TS - ta), 0.0f, TS);
    foc->dtc_b = CLAMP((TS - tb), 0.0f, TS);
    foc->dtc_c = CLAMP((TS - tc), 0.0f, TS);
}
...
void FOC_voltage(FOC_DATA *foc, float Vd_set, float Vq_set, float phase)
{
    ...
    // Modulation
    float V_to_mod = 1.0f / (foc->vbus_filt * 2.0f / 3.0f);
...
void FOC_current(FOC_DATA *foc, float Id_set, float Iq_set, float phase, float phase_vel)
{
    ...
    float mod_to_V = foc->vbus_filt * 2.0f / 3.0f;
    ...
    foc->power_filt = foc->vbus_filt * foc->i_bus_filt;
